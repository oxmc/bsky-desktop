...
electron_1.ipcRenderer.on('about-window:info', (_, info, app_name, version) => {
    ...
    if (info.custom_button) {
        const btns = document.querySelector('.buttons');
        // Loop through custom buttons if it's an array
        if (!Array.isArray(info.custom_button)) {
            const btn = document.createElement('button');
            btn.innerText = info.custom_button.label;
            if (info.custom_button.url) {
                btn.addEventListener('click', () => electron_1.shell.openExternal(info.custom_button.url));
                btn.classList.add('clickable');
            }
            if (info.custom_button.class) {
                btn.classList.add(info.custom_button.class);
            }
            if (info.custom_button.click) {
                btn.addEventListener('click', info.custom_button.click);
            }
            btns.appendChild(btn);
        } else {
            for (const custom_button of info.custom_button) {
                const btn = document.createElement('button');
                btn.innerText = custom_button.label;
                if (custom_button.url) {
                    btn.addEventListener('click', () => electron_1.shell.openExternal(custom_button.url));
                    btn.classList.add('clickable');
                }
                if (custom_button.class) {
                    btn.classList.add(custom_button.class);
                }
                if (custom_button.click) {
                    btn.addEventListener('click', custom_button.click);
                }
                btns.appendChild(btn);
            }
        }
